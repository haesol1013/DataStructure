public class BinaryTree<Key extends Comparable<Key>> {
    ...
    // 레벨 순회: 최상위 레벨부터 시작하여 좌에서 우로 노드 방문
    public void levelorder(Node<Key> node) {
        ArrayQueue<Node<Key>> queue = new ArrayQueue<>();
        Node<Key> t;
        // root 노드를 queue에 add
        queue.add(node);
        // queue가 빌 때까지 반복
        while (!queue.isEmpty()) {
            ...
            // left child 노드를 queue에 add
            if (t.getLeft() != null) {
                queue.add(t.getLeft());
            }
            // right child 노드를 queue에 add
            if (t.getRight() != null) {
